...
unsigned int g_mask_bit = 0xFFFFFFFF;
unsigned int g_mask_enb = 0;
...
/*
 * It was modified to allow the user to select the desired test.
 */
int get_menuselect(char argc, char** argv)
{
	char* menu_suffix;
	int menunum = 0;
	...
	if (optind >= argc) {
		...
		menunum = strtoul(argv[optind], &menu_suffix, 0);
		if ((menunum-1) > sizeof(tests)/sizeof(struct test))
			menunum = 0;
		if (*menu_suffix != '\0') {
			fprintf(stderr, "start menu suffixc %c \n", *menu_suffix);
...
/*
 * Only in the bit flip-test it was set to the master bit.
 */
int get_maskbit(char argc, char** argv)
{
	char* mask_suffix;

	optind++;
	g_mask_bit = 0xFFFFFFFF;
	g_mask_enb = 0;
	...
	if (optind >= argc) {
		...
		g_mask_bit = strtoul(argv[optind], &mask_suffix, 10);
		if (g_mask_bit > 31) {
			g_mask_enb = 0;
			return -1;
		} else {
...
/*
 * This patch was futher tested for selective
 * user wants
 */
...
int optional_check(int menunum, ulv* bufa, ulv* bufb, size_t count)
{
	int exit_code = 0;

	printf("Optional Check! \n");
	if ((menunum == MEMTEST_BITFLIP) && (g_mask_enb))
		printf("User Mask Bit: %d \n", g_mask_bit);
	/*
	 * Random value tests should be done
	 * unconditionally.
	 */
	printf("  %-20s: ", tests[0].name);
	if (!tests[menunum].fp(bufa, bufb, count))
		printf("ok\n");
	printf("  %-20s: ", tests[menunum].name);
	if (!tests[menunum].fp(bufa, bufb, count))
		printf("ok\n");
	else
...
int main(int argc, char **argv) {
    ...
    /* User Memtester Menu Select */
    menunum = get_menuselect(argc, argv);
    /*	Bit-Flip Test in Mask-bit */
    if (get_maskbit(argc, argv) == 0)
	    printf("User Mask Bit : %d \n", g_mask_bit);
...
//			exit_code |= EXIT_FAIL_OTHERTEST;
		...
		/* Optional Check */
		exit_code = optional_check(menunum, bufa, bufb, count);
		break;
	    }
            fflush(stdout);
        }
